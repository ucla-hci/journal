    ...
    for (id in menu) {
      ...
      if (title != null) {
        ...
        let short =
          ...
          "</p>";
        $("#entryTitles").append(
          '<div class="oneEntry">' + fg + short + "</div>"
        );
      }
    }
  }
...
function saveEntry() {
  ...
  _currentEntry = {
    data: { id, flag, title, content, date, marks, mouselog, keyboardlog },
    mark: { id, commentLog, commentSet, tagCount },
  };
}
...
$(".confirm-pop-up").on("click", function () {
  ...
  switch (last_menu_selection) {
    case 1:
      handelPrompt(sentence);
      break;
    case 2:
      handelReplace(sentence);
      break;
    case 3: {
...
$(".pop-up-selection").on("click", function () {
  ...
  if (current_sel_from != null && current_sel_to != null) {
    cm.markText(current_sel_from, current_sel_to, {
      className: id.replace(/\s/g, ""),
    });
    let from = analysisCoord(current_sel_from);
    let to = analysisCoord(current_sel_to);
    let command =
      "CD:(" + from.l + "," + from.c + "),(" + to.l + "," + to.c + ")," + id;
    log(command);
  }
  $("#cogndistortion").css("display", "none");
});
...
function handelPrompt(sentence) {
  ...
  if (compareCoord(start, end)) {
    ...
    if (cursor.findNext()) {
      ...
      // Not sure about sentences ending, add prompt alternativly
  ...
  toAppend.innerHTML =
    "<b>Prompt:</b> " + sentence + "<br> On: " + selectedText;
...
function handelReplace(sentence) {
  if (cm.somethingSelected()) {
    ...
    let e = analysisCoord(end);
    log(
      "replace:(" + s.l + "," + s.c + "),(" + e.l + "," + e.c + ")," + sentence
    );
    var toAppend = document.createElement("change");
    toAppend.innerHTML =
      "<b>Replacement:</b> " + sentence + "<br> On: " + selectedText;
    document.getElementById("changetracker").appendChild(toAppend);
  }
}
...
function handelFeedback(sentence) {
  ...
  let e = analysisCoord(end);
  log(
    "feedback:(" + s.l + "," + s.c + "),(" + e.l + "," + e.c + ")," + sentence
  );
  var toAppend = document.createElement("change");
  toAppend.innerHTML =
    "<b>Feedback:</b> " + sentence + "<br> On: " + selectedText;
  document.getElementById("changetracker").appendChild(toAppend);
}
